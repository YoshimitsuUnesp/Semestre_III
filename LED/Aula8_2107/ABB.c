...
// Verifica se a AB e uma arvore vazia
int Vazia(tree t)
{
   return (t == NULL);
}
...
{
   // o tipo 'tree' e um ponteiro de no!
...
// Funcao recursiva para verificar se uma AB e balanceada
boolean Balanceada(tree t)
{
   if (t == NULL)
      return TRUE;
   else if (t->esq == NULL && t->dir == NULL) // t nao tem filhos
      return TRUE;
   else if (t->esq != NULL && t->dir != NULL)            // t tem ambas subarvores nao-nulas
      return (Balanceada(t->esq) && Balanceada(t->dir)); // recursao
   else if (t->esq != NULL)                              // t tem um unico filho e na esquerda
      return (Altura(t->esq) == 1);
   else // t tem um unico filho e na direita
      return (Altura(t->esq) == 1);
}
